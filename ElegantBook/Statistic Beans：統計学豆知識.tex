...
\date{ 2026/3/4}
\version{0.3   {Typeset with \LaTeX{}}}
% \bioinfo{自定义题头}{信息}
...
\vspace*{\fill}
\begin{center}
    \large \textbf{\textit{For my friends.}}
\end{center}
\vfill  
...
\begin{itemize}
  ...
  \item Blog：\href{https://zesforu.pages.dev}{https://zesforu.pages.dev}
...
\begin{itemize}
  ...
  我们先假设今天万科喜家德没有喜三鲜，结果刚到喜家德坐下，发现隔壁桌吃的都是喜三鲜!\\
...
此书写给自己和我的朋友们，如果有些许助益的话，也算是满足了我小小的虚荣心。


\section{阅读建议}
以下是一些阅读本书的建议：
\begin{itemize}
  \item 当出现数学公式时，只需要知道这串符号在\textbf{算些什么}，也可主动了解，这不是坏事。
  \item 本书会在术语首次出现时解释说明，后不再赘述，建议添加标签、笔记。
  \item 符号标记为分析案例，建议动手尝试，不要只看不做，那没有用。
  \item 由于作者水平、认知有限，符号标记的内容仅为作者的见解，建议读者自查相关资料。
...
例如：假设你是一位日语教师，现想了解学校里日语学生的日语考试成绩。乍一看样本数据采集十分简单，只需收集一定量的日语成绩即可；但实际上
...
本附录包括了统计中常用到的一些分布，我们扼要论述了求和算子的各种性质，研究了线性和某些非线性方程的性质，并复习了比例和百分数。我们还介绍了一些在应用计量经济学中常见的特殊函数，包括二次函数和自然对数，前 4 节只要求基本的代数技巧，第 5 节则对微分学进行了简要回顾；虽然要理解本书的大部分内容，微积分并非必需，但在一些章末附录和第 3 篇某些高深专题中，我们还是用到了微积分。
